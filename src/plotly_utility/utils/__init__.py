...
__all__ = [
    ...
    "get_data",
    "to_numpy",
    "overlay_foreach_subplots",
]


def get_row_col(fig, xaxis, yaxis):
    table = npu.ja.apply(
        lambda ref: tuple(ref.trace_kwargs.values()), fig._grid_ref, 3
    ).squeeze()
...
def add_secondary_axis(
    ...
    i_data=1,
    anchor="x",
    secondary_axis_patch: dict = None,
    scale=1,
    add_hover_text=True,
):
    import copy
    ...
    setattr(
        fig.layout,
        f"{anchor}axis{new_axis_number}",
        getattr(fig.layout, f"{anchor}axis{this_axis_prefix}"),
    )
    ...
    if add_hover_text:
        ...
        var_name = (
            secondary_axis.title.text
            if secondary_axis.title.text is not None
            else f"secondary {anchor}"
        )
        x_suffix = (
            secondary_axis.ticksuffix if secondary_axis.ticksuffix is not None else ""
        )
    ...
    traces = np.array(
        [
            [
                ...
                for matched_traces in (
                    get_traces_at(fig, row, col) for col in range(1, n_col + 1)
                )
            ]
            for row in range(1, n_row + 1)
        ]
    )
    x, *other_unique_x = np.unique(
        [t.x for trace_rows in traces for t in trace_rows if t], axis=0
    )
...
def to_numpy(
    fig: go.Figure,
    flip_xy_if_any_bar_is_horizontal=True,
    x_order="left to right",
    y_order="top to bottom",
):
    if x_order is not None or y_order is not None:
        subplot_coordinates = subplots.get_subplot_coordinates(
            fig, x_order, y_order, flatten=False
        )
    ...
    traces_list = [
        get_traces_at(fig, row, col) for row, col in fig._get_subplot_coordinates()
    ]
    ...
    if n_rows == n_cols == 1:
        titles = (
            np.array(fig.layout.title.text)
    ...
    def get_point_data(traces):
        if flip_xy_if_any_bar_is_horizontal and any(
            trace["orientation"] == "h" for trace in traces if trace["type"] == "bar"
        ):
            return (
                tuple(
                    traces[i]["y"] if len(traces) > i else []
                    for i in range(max_n_traces)
                ),
                tuple(
                    traces[i]["x"] if len(traces) > i else []
                    for i in range(max_n_traces)
                ),
                tuple(
                    traces[i]["error_y"]["array"] if len(traces) > i else []
                    for i in range(max_n_traces)
                ),
                tuple(
                    traces[i]["error_x"]["array"] if len(traces) > i else []
                    for i in range(max_n_traces)
                ),
            )
        else:
    ...
    data = np.array(
        [
            (
                "",
                tuple(
                    traces[i]["name"] if len(traces) > i else ""
                    for i in range(max_n_traces)
                ),
    ...
    if hasattr(fig, "_fit_results"):
        ...
        data = npu.add_new_field_to(
            data, ("fit_result", fit_data.dtype, (fit_data.shape[-1],)), fit_data
        )
...
def overlay_foreach_subplots(
    fig_dict, colors: Optional[Union[dict, Iterable]] = None, opactiry=0.5
):
    first_key, *keys = fig_dict.keys()

    if colors is None:
        colors = dict(
            zip(fig_dict.keys(), itertools.cycle(px.colors.qualitative.Plotly))
        )
    ...
    def get_fig(key_):
        ...
        fig_.update_traces(
            name=key_, legendgroup=key_, marker_color=colors[key_], opacity=opactiry
        )
        return fig_
