    ...
    for fig in figs:
        ...
        if isinstance(fig, collections.abc.Sequence):
            ...
            if fig.layout.width is not None or fig.layout.height is not None:
                warnings.warn(
                    "not completely supported yet if fig.layout.width or fig.layout.height has value"
                )
        ...
        for trace in fig.data:
            if hovertext_format is not None and trace.hovertemplate is not None:
                vars = npu.ja.from_jagged_array(
                    [
                        m.split(":")
                        for m in re.findall(r"%{([^}]+)}", trace.hovertemplate)
                    ],
                    horizontal_size=2,
                )
                float_vars = vars[
                    [
                        npu.is_floating(
                            np.array(eval(f"trace.{var}", {"trace": trace}))
                        )
                        for var, *_ in vars
                    ]
                ]
                trace.hovertemplate = functools.reduce(
                    ...
                    zip(
                        np.char.add(
                            np.char.add("%{", npu.char.join(":", float_vars, axis=1)),
                            "}",
                        ),
...
def mpl_plot(fig, width=None, height=None, scale=1, engine="kaleido"):
    ...
    with io.BytesIO() as fp:
        ...
        fig.write_image(
            fp, format="png", width=width, height=height, scale=scale, engine=engine
        )
